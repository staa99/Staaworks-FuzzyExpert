...
namespace AForge.Fuzzy
{
    ...
    /// try
    /// {
    ///     var newAngle = IS.Evaluate( "Angle" );
    /// }
    /// catch ( Exception )
    /// {
    /// ...
    ...
    public class InferenceSystem
    {
        ...
        public float Evaluate (string variableName)
        {
            ...
            var fuzzyOutput = ExecuteInference( variableName );
            var res = defuzzifier.Defuzzify( fuzzyOutput, normOperator );
            return res;
        }
        ...
        {
            // gets the variable
            var lingVar = database.GetVariable( variableName );

            // object to store the fuzzy output
            var fuzzyOutput = new FuzzyOutput( lingVar );

            // select only rules with the variable as output
            var rules = rulebase.GetRules( );
            foreach (var r in rules)
            {
                if (r.Output.Variable.Name == variableName)
                {
                    var labelName = r.Output.Label.Name;
                    var firingStrength = r.EvaluateFiringStrength( );
                    if (firingStrength > 0)
                    {
